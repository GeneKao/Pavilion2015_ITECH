...
{
    public class GHC_PointDistributor : GH_Component
    {
        bool iReset = false;
        int iRandomSeed = -1;
        bool iPlay = false;
        ...
        public GHC_PointDistributor()
            : base(
                "Point Distributor",
                "Point Distributor",
                "Generate well-distributed seed points with varying density based on the principles of sea urchin plate growth",
                "Pavillion 2015",
                "Pavillion 2015")
        {
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            for (int i = iApicalDiscs.Count; i < pointAgents.Count; i++)
            {
                ...
                if (iBoundaryCurve.Contains(newPosition) == PointContainment.Outside)
                {
                    double t;
                    iBoundaryCurve.ClosestPoint(newPosition, out t);
                    newPosition = iBoundaryCurve.PointAt(t);
                }
            ...
            for (int i = iApicalDiscs.Count; i < pointAgents.Count; i++)
            {
                ...
                {
                    double t;
                    iBoundaryCurve.ClosestPoint(pointAgent.Position, out t);
                    double d = Utils.Distance(pointAgent.Position, iBoundaryCurve.PointAt(t));
                    if (d < minDist) minDist = d;
                }
        ...
        {
            get { return new Guid("{71cd3329-7def-44ac-9e14-f5b888eafab8}"); }
        }
    }
}
